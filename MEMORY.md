...
- **Digestório (UC-8) — 1º resumo do bloco gerado em 2026-08-19:** *Esôfago, estômago e intestino delgado* (Anatomia, Profa. Patrícia Castelucci, P1), slug `digest-anato-01-esofago-estomago-delgado`. 38 páginas (E1 com 19, 22 figuras cropadas do handout 2-up do slide), E2 com 30 (10 C/E, 3 "assinale a incorreta"), E3 com 5. Paridade Q01-Q30 dentro da banda em 20/20 MC. **Só Etapas 1 a 3** — deck-aula não montado (sem AnkiConnect alcançável em sessão remota). Entradas em `mapa-confusoes/`, `arquivos-trabalho/tema-card-*` e `arquivos-trabalho/integracao-mapa-*`.
- **Achado de pipeline 2026-08-19 — header do PDF atrasado uma página em toda transição de seção.** Com Typst 0.15.1, `state("etapa").get()` no `header` resolve no *início* da página, e o `set-etapa` dos helpers (`etapa-header`, `resumindo-page`, `sumario`, `gabarito-page`) roda depois do `pagebreak(weak: true)`, dentro do fluxo da página nova. Resultado: a página do Resumindo saía com header "ETAPA 1", a de abertura da E2 com header "RESUMINDO", e assim por diante — `auditar_pdf_visual.py` bloqueia com "header dessincronizado". **Workaround aplicado neste resumo:** `#set-etapa("...")` emitido no `main.typ` imediatamente ANTES de cada helper que quebra página. **Fix durável pendente (precisa de OK do Davi):** mover a linha para o `HEADER` de `gerar_main.py` ou trocar o `set-etapa` interno dos helpers para antes do `pagebreak` no template.
...
### Aberto 2026-08-19 (Digestório — Esôfago/Estômago/Delgado)
- **Header dessincronizado no PDF (fix durável).** Ver § Active Projects. Escolher entre (a) `gerar_main.py` emitir `#set-etapa(...)` antes de cada helper que quebra página — mudança de 6 linhas, não toca o template; (b) mover o `set-etapa` para antes do `pagebreak` dentro de `etapa-header`/`resumindo-page`/`sumario`/`gabarito-page` no `nebli_v2_apostila.typ` — mais limpo, mas altera o template canônico e exige amostra antes/depois. Decisão do Davi.
- **`auditar_pdf.py` — contagem de palavras da E1 falseada pelo `#mini-resumo`.** O delimitador de fim do miolo procura a string "Resumindo", e o helper `#mini-resumo` renderiza "Resumindo até aqui:" já na 2ª página da E1. Resultado: reportou 769 palavras num miolo de ~7.600. Fix: procurar o banner de fim por "Resumindo" em linha isolada / no header, ou usar "Etapa 2" como delimitador primário. Mesmo efeito colateral gera os 2 warnings de "banner de resumindo no corpo" no `auditar_pdf_visual.py`.
- **`auditar_pdf_visual.py` — check de cores da E2 usa nomes de helper aposentados.** Procura `#prova-consolidacao`/`#prova-integracao`/`#prova-aplicacao`, que não existem mais no template (a E2 usa `#questao-mc`/`#questao-ce` com `badge-*`). Warning é falso-positivo em todo resumo novo; atualizar os nomes no check (e a linha § Cores do `ROLES.md`, que também está desatualizada).
- **Deck-aula do Digestório 01 pendente.** Corrida foi só Etapas 1 a 3; AnkiConnect não é alcançável de sessão remota (ver `referencias-externas/INFRA-REMOTO.md`). Rodar a curadoria + `montar_deck_aula.py` numa sessão local.
- **Upload pro Drive pendente** (PDF + slide na pasta `Esôfago, Estômago e Intestino Delgado`) — gated em `rclone config`.
