...
- **Header de etapa atrasado em uma página (`set-etapa` depois do `pagebreak`) — fix proposto, aguardando aprovação.** `#etapa-header(titulo)` chama `pagebreak(weak: true)` e só depois `set-etapa(titulo)`; como o header da página é resolvido no início dela, a **primeira** página de cada etapa herda o nome da seção anterior (Resumindo com header "Etapa 1", Etapa 2 com header "Resumindo"). O `auditar_pdf_visual.py` tolera o par (etapa1→resumindo) mas **bloqueia** (resumindo→etapa2). **Workaround aplicado em `fisio-tgi-01-motilidade`:** emitir `#set-etapa("...")` no `main.typ` imediatamente ANTES de cada `#etapa-header(...)` e antes do `#include "resumindo.typ"` — resolve os três boundaries, e a página anterior não muda (o header dela já foi resolvido). **Fix real (1 linha, precisa de OK do Davi + amostra antes/depois, regra da pasta `typst-template/`):** inverter as duas linhas dentro de `etapa-header` (`set-etapa` antes do `pagebreak`). Se aprovado, `gerar_main.py` volta a gerar `main.typ` sem o workaround.
...
### 2026-08-24
- Resumo **Motilidade do Tubo Digestório** (`fisio-tgi-01-motilidade`) fechado — 1º de Digestório (UC-8). Nenhuma regra canônica nova; duas observações operacionais registradas em § Pendências (header de etapa atrasado) e abaixo.
- **Formato do `mapa-confusoes/<slug>.md`:** o `precompile-check.py` conta confusões por `CONFUSAO_ITEM_RE = ^\s{0,3}\d+\.\s+\*\*` — ou seja, **lista numerada** (`1. **Nome.**`), não lista com hífen. Mapa escrito com `- **Nome.**` é lido como 0 confusões e reprova o gate.
- `auditar_pdf.py` § "Palavras da E1" subconta grosseiramente (reportou 621 numa E1 de ~7.500 palavras) — é warning, não bloqueia; conferir pelo `pdftotext` das páginas da E1 quando importar.
