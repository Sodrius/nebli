...
### 2026-08-25
- **Teto da E1 cai de 22 para 15 páginas, cumprido por CONCISÃO (pedido de Davi: "quero, a partir de agora, etapas 1 com até 15 pgs" + "quero que concisão seja a forma de ficar em 15 páginas, mantenha regra de aprofundar + completude; pode ser mais direto e mais didático").** Vale para todo resumo daqui pra frente. **A 1ª versão que escrevi desta regra estava errada** — propus estreitar o recorte e fundir subtópicos como alavanca principal; Davi corrigiu na hora. **Cobertura completa do slide, passe de aprofundamento (≈1 injeção por subtópico) e conclusão integradora são INVARIANTES**; o que encolhe é *palavras por ideia*, nunca *quantidade de ideias*. Orçamento: 10-12 subtópicos de ~350-420 palavras, 9-12 figuras, ~4.000-5.000 palavras. Alavancas (ordem de rendimento): preâmbulo antes do mecanismo · parágrafo que reafirma o anterior · moldura conversacional (Categoria 17 cai para ≤1 por subtópico) · legenda que reexplica o texto · `#mini-resumo` ritual · catálogo em prosa que devia ser tabela · adjetivo de reforço. Teste de fechamento: *que passo mecanístico novo este parágrafo entrega?* Wiring: `CLAUDE.md` § Rules globais, `ROLES.md` § Redator diretriz 2 + § Orquestrador Seção A, `ERROS.md` #7 + tabela + F4.
- **`auditar_pdf.py` ganha `check_paginas_e1`** — o teto era "manual/warn" e agora é medido no PDF (aviso, não bloqueio). Faixa de palavras da E1 mantida em 3.500-5.000: é o alvo de uma E1 de ≤15 páginas com cobertura completa e prosa concisa. Estourar ali quase sempre é verbosidade, não excesso de conteúdo.
