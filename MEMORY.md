...
### 2026-08-20
- **Resumo gerado — Reconhecimento inato no sistema imune (`imuno-02-reconhecimento-inato`, Imunologia).** Etapas 1 a 3 completas a partir de **transcrição de aula em PDF** (não de slide solto): 22 slides embutidos extraídos do próprio .docx→PDF via PyMuPDF para `figuras/imuno-02-reconhecimento-inato/`, com `MAPA_CONTEUDO.txt` montado pareando cada imagem à narração da página seguinte. PDF de 42 págs (E1 = 22 págs, no teto), 17 figuras, 8 termo-notas, 25 siglas. **Todos os gates verdes:** paridade Q01–Q30 30/30 PASS (média 1.07, máx 1.23) sem nenhuma remediação; 10 C/E com 4 itens; mapa de confusões 12×5; `auditar_pdf` e `pos_pipeline_check` 0 erros. Cobertura E1→E2: 12/12 subtópicos com ≥2 questões (3.3 saiu com 1 na 1ª passada e foi corrigido trocando o item III de Q12 — "truque do 4º item de C/E" do ROLES). Índice de completude: E1×slide **9**, E2×E1 **9**, Cards×E1 **n/a**. **Deck-aula NÃO montado** — passo 11 do `/resumo` exige AnkiConnect em localhost, inalcançável de sessão remota; fica para corrida na máquina do Davi. Relatório completo em `arquivos-trabalho/relatorio-fechamento-imuno-02-reconhecimento-inato.md`.
- **Ambiente remoto não vinha com `typst` nem `poppler-utils`** — instalados na sessão (typst 0.13.1, release musl; poppler via apt). Candidato a hook de SessionStart se sessões remotas virarem rotina.
- **Bug de auditor identificado (não corrigido):** `auditar_pdf.check_palavras_e1` delimita o miolo da E1 pela substring "Resumindo", que colide com o "Resumindo até aqui:" impresso pelo helper `#mini-resumo` — reporta "E1 curta" (669 palavras) em qualquer resumo com mini-resumo. Warning não-bloqueante. Correção de 1 linha: ancorar no banner, não na substring solta.
