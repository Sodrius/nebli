...
### Aberto 2026-08-24 (bloqueado em ação do Davi)
- **Credencial do AnkiWeb para o acesso pela nuvem.** A infra está pronta e testada (`INFRA-REMOTO.md` § 0), mas nenhuma sessão na nuvem toca a coleção real até existir `ANKIWEB_HKEY` (ou `ANKIWEB_USER`/`ANKIWEB_PASS` no primeiro login, que imprime o hkey) como **env var do environment** do Claude Code — mesma mecânica do `NEBLI_DRIVE_SA_JSON`, uma vez só. Antes do primeiro uso: sincronizar PC/celular com o AnkiWeb. Falta ainda validar contra a coleção real: tempo do full download inicial, e o peso do `NEBLI_ANKI_SYNC_MEDIA=1` para Image Occlusion.
...
### 2026-08-24
- **Acesso ao Anki a partir de sessão na nuvem (pedido de Davi: "queria que você conseguisse acessar o anki pra fazer curadoria anking e deck injection direto" — do tablet).** Cai a premissa do `INFRA-REMOTO.md` de que "a nuvem não serve": a sessão não alcança o `localhost` do PC (confirmado: connection refused), mas passa a ter **a própria coleção**, baixada do AnkiWeb pela lib oficial `anki`. Três peças em `flashcards/scripts/`: `anki_collection_backend.py` (36 ações na superfície exata do AnkiConnect, sobre `Collection` + `sync_collection`), `ankiconnect_local.py` (**AnkiConnect emulado** em `127.0.0.1:8765` — faz os ~31 scripts com `urllib` próprio funcionarem sem edição, e mantém a coleção aberta uma vez por sessão em vez de uma por script), `nebli_anki.py` (o antigo `anki.py`, agora com transporte `connect|collection|auto`). Os 31 scripts que importavam o helper migraram para `from nebli_anki import call`. **Rename obrigatório, não estético:** `anki.py` ao lado dos scripts sombreia a lib `anki` (ver `ERROS.md` item 21). **Segurança:** sync que exigiria `FULL_UPLOAD`/`FULL_SYNC` aborta — a nuvem nunca decide qual lado da coleção sobrevive; `FULL_DOWNLOAD` só com coleção local vazia. **Credencial:** `ANKIWEB_HKEY` como env var do environment (mecânica do `NEBLI_DRIVE_SA_JSON`), uma vez e vale para todas as sessões. **Ganho:** o `guid` — que o AnkiConnect não expõe — existe aqui, então o casamento card-a-card do apply vira identidade exata em vez de texto bruto (`raw0`). Validação: `testar_backend_anki.py` 38/38 em coleção descartável + E2E com `configurar_ritmo_anking.py` (ritmo 15/dia gravado) e `dessuspender_anking.py` (5 cards dessuspendidos por tag `#AK_Step1_v12::`), ambos scripts com `urllib` próprio, sem uma linha editada. **Pendente:** credencial do AnkiWeb (Davi) para validar contra a coleção real; mídia (Image Occlusion) exige `NEBLI_ANKI_SYNC_MEDIA=1`.
