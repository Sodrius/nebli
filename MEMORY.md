...
### 2026-08-27 · "Pré-aula" vira 2ª parte canônica do Resumindo
- **Pedido de Davi em duas rodadas no mesmo dia.** Primeiro: *"adiciona uma segunda parte do resumindo como canônica: vai ser um resumo 100% do zero para leigos do assunto, alguém que não leu a E1 e quer acompanhar melhor a aula."* Entreguei como **glossário** ("Do zero"). Davi corrigiu a forma: *"quero que essa funcionalidade chame **pré-aula** e seja um **texto corrido**, linguagem NEBLI, aprofundamento correto, de **até 2 páginas**, com as informações mais importantes em **negrito preto**"* — e pediu para **não refazer os resumos já fechados**, só deixar canonizado para os próximos.
- **Lição que fica registrada:** mesmo numa seção cuja função é "dar nome às coisas", a forma NEBLI é **prosa**, não lista. Eu li "principais termos de forma sucinta" como pedido de glossário; era pedido de prosa introdutória que *contém* os termos. A regra "prosa expositiva precede tudo" vale também aqui.
- **Forma canônica:** texto corrido em voz NEBLI, teto de 2 páginas (~1.000–1.200 palavras), coluna única justificada, corpo 9.8pt, banner teal. Os termos entram dentro da frase que os explica, na ordem em que o professor vai usá-los. Linguagem do dia a dia e zero pressuposto, mas com aprofundamento correto — simplificar não autoriza errar. Negrito preto no que o leitor leva embora (o template já força `show strong: set text(fill: text-main)`). Sem sigla nova, sem `#termo-nota`, sem footnote, sem remissão a outro resumo.
- **Implementação:** helper `#pre-aula-page(conteudo)` no template (backups `nebli_v2_apostila.typ.bak-2026-08-27-A1` e `-A2`), arquivo `typst-build/pre-aula.typ` entregue junto do `resumindo.typ`, e o `gerar_main.py` inclui a seção sozinho quando o arquivo existe — retrocompatível. O helper de glossário `#resumindo-zero-page` **fica no template só como retrocompat** (não usar em resumo novo), para que os 2 PDFs de UC3 fechados com ele sigam recompilando. Wiring: `CLAUDE.md` § Resumindo Parte 2 + § Ordem do PDF 4b, `ROLES.md` § Redator diretriz 5.bis, `TEMPLATE_API.md`, `.claude/commands/resumo.md` passo 6.
- **Sem retrofit, por decisão de Davi.** Os dois PDFs de UC3 já entregues (`patologia-edema-congestao` 41 pp e `patologia-ambiental` 37 pp) ficam com a versão glossário "Do zero". A Pré-aula em prosa começa a valer do próximo resumo em diante. Exemplar de referência já escrito e validado em `arquivos-trabalho/exemplares-pre-aula/pre-aula-patologia-edema-congestao.typ` (2 páginas exatas) — serve de few-shot para as próximas corridas.
- **Pendência do header-sync BAIXADA na mesma leva.** Como o `gerar_main.py` precisava mudar de qualquer forma, o par `#pagebreak(weak: true)` + `#set-etapa(...)` entrou no `HEADER` de vez, antes de cada `#etapa-header`, do Resumindo, da Pré-aula e do gabarito. O header de página deixa de atrasar uma página em toda transição de seção, e o patch manual no `main.typ` não precisa mais ser reaplicado. Backup: `backups/tecnicos/gerar_main.py.bak-2026-08-27-dozero`.

### 2026-08-27
- **Resumo `patologia-ambiental` (UC3 · Patologia Ambiental, mesma aula-irmã do PDF de Edema), Etapas 1 a 3 guiadas pelo Robbins Cap. 9, dial `profundidade: fundo`.** Segunda metade do PDF anexado (**páginas 17-51**, extraídas para `figuras/patologia-ambiental/slide-01..35`). Aula ainda mais visual que a de Edema: 35 slides quase todos gráfico, mapa, foto ou fluxograma, com fontes de literatura (Logan 1953, Pope 2000, Salvi 2009, Soares/Atherosclerosis, Saldiva/Bogliolo, IPCC 2021) e quase nenhum texto projetado. E1 com 6.373 palavras / 35 páginas, 13 figuras, 9 termo-notas, 2 siglas; E2 PROFUNDO 8/17/5 com 10 C/E; paridade Q01-Q30 na banda (média 1,02, máx 1,13) e também dentro da métrica complementar do `precompile-check` (correta × mediana, 0,80-1,30).
- **Três achados desta corrida que valem como aprendizado de processo.** (1) **O `pos_pipeline_check` pegou vocabulário banido meu:** o título da PARTE III no sumário do Tema Card dizia "artéria, população e *prova*" (eu quis dizer prova = evidência), e o check `\bProva\s*[123]\b` casou com "prova" + o "3.1" da linha seguinte. Corrigido para "método" — e fica o registro de que o check de capa também funciona como rede contra a palavra banida em qualquer lugar das 2 primeiras páginas. (2) **Legenda descrevendo figura que não existe:** escrevi "as barras urbanas ficam duas a três vezes acima do alvo" para o slide-18, que **não tem barras** — é um mapa da América do Sul com três regiões metropolitanas anotadas (16, 20 e 28 µg/m³) contra a diretriz da OMS. Pego na validação visual obrigatória, não por script. **Reflexo a fixar: abrir a figura antes de escrever a legenda, sempre — não redigir a partir do texto extraído do slide.** (3) **Órfãs de página em PARTE figura-densa:** subtópico 1.4 deixou 2-4 linhas sozinhas numa página e o Resumindo de 10 seções estourou para uma 2ª página quase vazia. Resolvidos por `#figura-lateral` no mapa (o helper certo para figura alta e estreita), largura menor no gráfico do IPCC e condensação do Resumindo para 8 seções. 35 páginas no fim, sem página de mancha baixa exceto o gabarito.
- **Confirmado nesta 2ª corrida que os 2 falso-positivos de auditor são sistêmicos** (já registrados em `PENDENCIAS.md`): `auditar_pdf.py` acusou "E1 com 1.026 palavras" contra 6.373 reais (para no 1º `#mini-resumo`), e `auditar_pdf_visual.py` acusou "helpers de cor faltando" procurando helpers da E4 removida. O patch de header-sync no `main.typ` foi reaplicado à mão nesta corrida — mais um argumento para levá-lo ao `gerar_main.py`.

### 2026-08-27 · Edema e Congestão
- **Resumo `patologia-edema-congestao` (UC3 · Edema e Congestão, Prof. Luiz Fernando Ferraz da Silva), Etapas 1 a 3 guiadas pelo Robbins Cap. 4, dial `profundidade: fundo`.** Slide anexado tinha 51 páginas mas só as **16 primeiras** eram da aula (da 17 em diante começa "Patologia Ambiental") — recorte declarado na Seção A do Tema Card. Slide quase todo imagem (macro/micro de autópsia + 4 fluxogramas em inglês), então a régua conceitual veio do Robbins, como Davi pediu. E1 com 7.220 palavras / 39 páginas, 12 figuras, 11 termo-notas; E2 PROFUNDO 8/17/5 com 10 C/E; paridade Q01-Q30 toda na banda (média 1,05, máx 1,21). **UC3 não tem `banco/aulas_uc03.yml`** — sem calibração de banco FMUSP nesta corrida. Sem curadoria de cards: sessão remota, sem AnkiConnect.
- **Três defeitos de pipeline encontrados e corrigidos nesta corrida.** (1) **Rótulos do gabarito consolidado eram fixos** (`Q01–Q10 / Q11–Q25 / Q26–Q30`) em `gerar_main.py`, então mentiam em qualquer distribuição que não fosse PADRÃO — num resumo PROFUNDO (8/17/5) o bloco "Consolidação (Q01–Q10)" listava Q01–Q08 e a "Integração (Q11–Q25)" começava em Q09. **Corrigido na origem:** `render_gabarito` agora deriva a faixa dos números presentes em cada bloco (backup em `backups/tecnicos/gerar_main.py.bak-2026-08-27-edema`). (2) **Header de página dessincronizado em TODA transição de seção** — `etapa-header` faz `pagebreak(weak: true)` e só então `set-etapa`, e o Typst resolve o header da página no início dela, então a 1ª página de cada seção carrega o nome da seção anterior (reproduzido em `typst-build/_test_header.typ`, sem conteúdo meu). Corrigido **localmente no `main.typ` deste resumo** (`#pagebreak(weak: true)` + `#set-etapa(...)` antes de cada `#etapa-header`, do `#include resumindo.typ` e do `#gabarito-page`) — o fix definitivo em `gerar_main.py`/template está em `PENDENCIAS.md` aguardando Davi. (3) **Página fantasma** entre E2 e E3 (spill de espaçamento da última questão) — resolvida encurtando 2 distratoras de Q30 sem sair da banda.
- **Dois warnings dos auditores são falso-positivo conhecido, não defeito do resumo.** `auditar_pdf.py` acusou "palavras E1: 1005" porque delimita o miolo até a 1ª ocorrência de "Resumindo" — e o helper `#mini-resumo` imprime "Resumindo até aqui:" já na p5; a E1 real tem 7.220 palavras. `auditar_pdf_visual.py` acusou "helpers de cor faltando" porque procura `#prova-consolidacao/integracao/aplicacao`, que são helpers da **E4 removida em 2026-05-22**; a E2 usa `badge-*`. Os dois checks estão desatualizados (ver `PENDENCIAS.md`).
