...
- **Deck-aula de `anato-03-intestino-grosso-canal-anal` (2026-08-20):** montar em sessão **local** com Anki aberto — curadoria AnKing por conceito, decks externos para o eixo visual/anatômico (BlueLink cadáver/IO é a fonte natural aqui) e NEBLIcards autorais para as lacunas tipo-a. Fonte-verdade dos conceitos = E1 já redigida (12 subtópicos). Bloqueio foi ambiental (AnkiConnect em localhost, sessão remota), não editorial.
...
### 2026-08-20
- **Resumo `anato-03-intestino-grosso-canal-anal` (Digestório UC-8 · Anatomia · Profa. Patrícia Castelucci) — Etapas 1 a 3 fechadas.** Fontes: slide de 27 páginas (2 slides por página) + ROTEIRO de aula prática + Perguntas orientadoras (2 casos clínicos do docente). PDF `resumos-gerados/Intestino Grosso e Canal Anal - Etapas 1 a 3 - PC.pdf` (42 pp, 31,0 MB) + cópia leve (4,9 MB), seguindo a nomenclatura única de 2026-07-12 (`<aula curta> - Etapas 1 a 3 - <sigla do prof>`).
- **Novo padrão de extração de slide 2-up (aplicado nesta corrida):** o slide da UC-8 traz **dois slides por página de PDF**, então `extrair_slides.py` sozinho produz figuras que misturam dois assuntos. Solução adotada: recorte automático das metades superior/inferior em `figuras/<slug>/sNNa.png` / `sNNb.png` (300 dpi, com trim de margem branca), e a E1 referencia as metades, não a página inteira. Vale como receita para qualquer slide em formato de apostila 2-up.
- **Deck-aula não montado nesta corrida (limitação de ambiente, não de regra).** O gate de 2026-08-03 exige fechar o deck-aula dentro do `/resumo`, mas a sessão rodou em **ambiente remoto**, e AnkiConnect vive em `localhost` da máquina do Davi (`referencias-externas/INFRA-REMOTO.md` já registra que cloud agents não alcançam localhost). Curadoria AnKing + deck-aula ficam **pendentes** para uma sessão local com Anki aberto; a E1 já está escrita no nível de profundidade que o deck vai exigir.
