...
- **Resumo de Patologia — Lesão celular (2026-08-24):** gerado a partir de **transcrição integral da aula em PDF** (sem slides em imagem) — `patologia-01-lesao-celular`, 36 páginas, E1 de 17 páginas com 11 figuras **autorais em Typst nativo** (esquemas de cascata, hub do cálcio, curva de reperfusão, 2 tabelas comparativas), já que não havia slide para extrair. Etapas 1 a 3 fechadas; deck-aula NÃO montado (sessão remota não alcança o AnkiConnect local). Precedente útil: entrada por transcrição funciona bem e o esquema Typst nativo resolve a exigência de figura sem slide (entrada de peso alto do § Diário de revisões).
...
- **`auditar_pdf.py` conta as palavras da E1 errado quando há `#mini-resumo` (achado 2026-08-24).** O delimitador de fim do miolo é a primeira ocorrência da string `Resumindo` no texto do PDF — e o helper `#mini-resumo` imprime "Resumindo até aqui:", então a contagem para no primeiro mini-resumo da PARTE I. Em `patologia-01-lesao-celular` reportou 847 palavras onde havia ~8.300. **Fix (~2 linhas):** delimitar pelo banner da página Resumindo (procurar a última ocorrência, ou usar `Etapa 2` como limite primário). É warning, não bloqueia — mas o número reportado é inútil hoje.
