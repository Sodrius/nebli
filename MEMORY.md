...
### 2026-08-31 (corrida `micro-02-fisiologia-crescimento-bacteriano`)
- **Resumo `micro-02-fisiologia-crescimento-bacteriano` gerado** (Fisiologia e crescimento bacteriano, Profa. Elisabete Vicente, ICB/USP — **segunda aula de Microbiologia da UC03**, conteúdo 27, prova P2). Dial **`profundidade: fundo`** a pedido do Davi ("etapas 1 a 3 num nível um pouco acima da prova"). 46 páginas: capa · "Antes da aula" (2p) · sumário (1p) · E1 com 22 páginas e 16 subtópicos · Resumindo em 2 páginas · 30 objetivas · 5 discursivas · gabarito. 17 figuras, todas do slide. Passo 2b da calibração UC03 rodou (11 subquestões de MB no acervo 2015-2025 — uma das aulas mais cobradas do componente). **Deck-aula do passo 11 não rodou** — sessão remota não alcança o AnkiConnect local (mesmo bloqueio das corridas do fígado e do `micro-01`).
- **Recorte declarado: a seção 7 do slide ("Controle do crescimento bacteriano") ficou de fora.** O próprio slide 34 remete o tema à aula seguinte; desinfetante, antisséptico e antibiótico entram na E1 apenas como consequência do que a curva ensina (agente que exige parede em construção só age na fase de multiplicação), em uma a duas frases, sem subtópico nem figura própria. Fronteira registrada na Seção A do Tema Card.
- **`ERROS.md` #23 confirmado na prática (gabarito C/E divergente).** A passada manual item a item pegou **Q14 marcada CECC quando a verdade real das assertivas era CEEC** — divergência que nenhum gate automático detecta (o `precompile-check` audita paridade de MC e distribuição de letras; o `verificar_gabarito_resumo` confere numeração e formato, não semântica). O procedimento do #23 (redigir as assertivas → ler item a item → só então aplicar a guarda anti-degeneração) funcionou como rede. Nas 10 C/E: 23 C e 17 E, sem `CCCC`/`EEEE`, sem `CECE`/`ECEC` puro.
- **Alavanca 1 do F4 aplicada por orçamento de páginas.** Com 17 figuras e um miolo de 10.5k palavras, a E1 fechava em 24 páginas. A faixa de largura caiu para 50–62%, com 70–72% reservado às três estruturais (curva de crescimento, tubos de tioglicolato, painel dos cinco tipos de meio), e a figura `slide-25` (mapa geral de biossíntese) foi descartada pelo filtro de utilidade pedagógica — na largura que cabia ficava ilegível, e o mecanismo dela já estava resolvido em prosa. Resultado: 22 páginas, exatamente no teto.
