...
### Aguardando sanção do Davi (2026-08-23)
- **Fix de header defasado no `#etapa-header` (template canônico).** `set-etapa` passou a vir ANTES do `pagebreak(weak: true)` — sem isso, a 1ª página de cada etapa imprimia o cabeçalho da etapa anterior (abertura da E1 com "SUMÁRIO", abertura da E2 com "RESUMINDO", gabarito com "ETAPA 3"), e o `auditar_pdf_visual.py` bloqueava a movimentação do PDF no caso resumindo→etapa2. Testado antes/depois em `biomol-recombinacao-vdj-imunoglobulinas` (41 páginas idênticas nos dois builds, só o cabeçalho muda). Backup: `backups/tecnicos/nebli_v2_apostila.typ.bak-2026-08-23-header-sync`. **Reverter = trocar as duas linhas de volta.** Detalhe em `ERROS.md` § 21.
- **Delimitação do miolo da E1 nos auditores.** `auditar_pdf.py` (contagem de palavras) e `auditar_pdf_visual.py` (header sincronizado) usam a string solta "Resumindo" como fronteira, e o helper `#mini-resumo` imprime "Resumindo até aqui:" no meio da E1 — daí contagem de palavras absurdamente baixa (1878 numa E1 de ~8900) e warnings de "banner de resumindo no corpo". Trocar a heurística pelo banner real do `#resumindo-page`. Não bloqueia nada hoje, só polui o relatório.
- **Check de cor da E2 no `auditar_pdf_visual.py` está fóssil.** Ele procura `#prova-consolidacao/integracao/aplicacao` (helpers da E4, removida em 2026-05-22); a E2 canônica usa `badge-consolidacao/integracao/aplicacao` dentro de `#questao-mc`. Resultado: warning "helpers de cor faltando" em todo resumo novo, mesmo com as cores corretas no PDF.
...
### 2026-08-23
- **Resumo `biomol-recombinacao-vdj-imunoglobulinas` gerado (Etapas 1 a 3, foco em didática).** Aula de Biologia Molecular do Prof. Alexandre Bruni-Cardoso (IQ-USP): recombinação somática e geração de diversidade de imunoglobulinas. 41 páginas, E1 em 20 páginas com 20 figuras (19 recortes de painel único feitos do PDF do professor + 1 slide inteiro). Sem slug no banco (aula fora das UCs com calibração) e **sem deck-aula** — a sessão rodou em container remoto, onde o AnkiConnect (localhost) é inalcançável; passo 11 do `/resumo` fica pendente por impossibilidade técnica.
- **Fix de header defasado no `#etapa-header`** (aguardando sanção — ver § Pendências e `ERROS.md` § 21).
- **Achado de processo:** a passada de conferência item-a-item do gabarito C/E pegou **5 assertivas de 40** cujo valor de verdade não batia com a chave sorteada (Q03-III, Q18-III, Q18-IV, Q24-IV, Q30-IV) — todas depois de o `verificar_gabarito_resumo.py` ter passado com 0 aviso. O script confere *forma* (nº de letras, letra existente, correta não-placeholder), não *verdade*. Reforça por que o revisor-gabarito existe como passada separada.
