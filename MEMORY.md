...
- **UC03 ativa no 2º semestre (assimilada 2026-08-27):** **UC03 — Princípios do Desenvolvimento das Doenças** (FMUSP/ICB/IQ) roda de 04/08 a 04/12/2026, em paralelo com UC16/UC08/UC04/UC19/UC21. Componentes: Patologia (PT), Imunologia (IM), Microbiologia (MB), Radiologia (RD), Biologia Molecular/Bioquímica (BM/BQ) + Grand Rounds. **Provas: P1 28/08 · P2 02/10 · P3 06/11 · P4 04/12.** Cronograma em `banco/aulas_uc03.yml` (76 entradas; P1/P2 da tabela oficial da planilha do Davi, P3/P4 reconstruídos do calendário mensal e marcados `provisorio: true`). Calibração por provas antigas (30 provas 2015-2025, 650 subquestões) em `referencias-externas/uc03/` — plugada no `/resumo` passo 2b, **só para UC03**.
...
### 2026-09-02 (corrida `micro-04-controle-microbiologico`)
- **Resumo `micro-04-controle-microbiologico` gerado** (Controle do crescimento microbiano — esterilização, desinfecção e antissepsia; Profa. Elisabete Vicente, ICB/USP, slide de 2023 com 34 páginas — **quarta aula de Microbiologia da UC03**, conteúdo 30, P2). Dial **`profundidade: fundo`** a pedido do Davi ("roda as etapas 1 a 3 acima do nível da prova"). 47 páginas: capa · "Antes da aula" (2p, 1.081 palavras, 87 termos plantados) · sumário (1p) · **E1 com 21 páginas e 15 subtópicos** (dentro do teto de 22) · Resumindo em 1 página (9 seções) · 30 objetivas (20 MC + 10 C/E + 3 "assinale a incorreta") · 5 discursivas · gabarito. 15 figuras, todas do slide. Passo 2b da calibração UC03 rodou (1 subquestão de MB no acervo — 2025 P2 3.3, reprocessamento de material com *M. tuberculosis*; degrau declarado em 8 pontos, do mecanismo do vapor saturado ao critério de Spaulding, que o slide não traz). Paridade 20/20 PASS (média 1,01). Relatório completo em `arquivos-trabalho/fechamento-micro-04-controle-microbiologico.md`.
- **Bug de infra corrigido — os 9 stubs de `.claude/agents/` declaravam `tools:` em minúsculas.** `read`, `bash`, `write`, `edit` e `grep` não resolvem para nenhuma tool, e o spawn é recusado com "would be spawned with zero tools". Isso derrubava `revisor-gabarito`, `revisor-densidade`, `revisor-didatico`, `revisor-profundo`, `revisor-pdf-*`, `cadernista`, `compilador` e `orquestrador` — provavelmente a causa real de parte do que o `ERROS.md` F9/#3 atribuía a incompatibilidade Opus+Agent tool. Todos normalizados para `Read`/`Bash`/`Write`/`Edit`/`Grep`. As definições carregam no início da sessão, então a correção só vale da próxima corrida em diante; **testar `revisor-gabarito` e `revisor-densidade` na próxima e, se subirem, reavaliar o F9**.
- **Quebra declarada:** densidade de ≈617 palavras por subtópico contra o alvo 450–550 do `ERROS.md` F10. O teto de 22 páginas foi respeitado (21) e os cinco desperdícios do F10 foram caçados; o excedente é conteúdo do dial `fundo`. Decisão sobre apertar mais fica com o Davi.
- **Deck-aula do passo 11 não rodou** — sessão remota não alcança o AnkiConnect local (mesmo bloqueio das corridas `micro-01`, `imuno-07` e do fígado). Por isso o eixo "Cards × E1" do Índice de completude saiu como não avaliável, e o passe de aprofundamento nasceu de slide + bibliografia + calibração UC03, não de cards.
- **Ambiente remoto:** `typst` 0.15.1 e `poppler-utils` instalados na sessão, e `pymupdf` via pip (ERROS #22). Desta vez `apt-get update && apt-get install poppler-utils` funcionou.

### 2026-08-31 (corrida `imuno-07-inflamacao-inicio-resolucao`)
- **Economia de palavra na E1 vira canônica** (pedido de Davi no fechamento desta corrida; primeira leitura minha foi errada — ele falava da apostila, não do chat). Mesma profundidade, menos palavra: cortar moldura, legenda redundante, achado recontado e conclusão que reconta; mecanismo intocável. Alvo 450–550 palavras/subtópico (esta corrida saiu com ~660) e teto de 22 páginas de volta ao sério. Gravado em `CLAUDE.md` § Preferences e `ERROS.md` F10.
...
- **Ambiente remoto:** `typst` e `poppler-utils` instalados na sessão (ERROS #22); desta vez o `apt-get install poppler-utils` funcionou após `apt-get update`, sem necessidade dos shims sobre pymupdf.

### 2026-08-29
- **"Antes da aula" vira GATE HARD (pedido de Davi: "coloca como gate-hard — 2 pgs, prosa fluida, tópicos mais importantes").** A seção era canônica desde 2026-08-28 mas o `precompile-check.py` só a auditava **se o arquivo existisse**, o que na prática a deixava opcional. Agora: ausência de `typst-build/pre-aula.typ` é **erro bloqueante** (exit ≠ 0), e a nova `check_pre_aula` reprova a seção que não cumpre a forma canônica — fora de 800–1300 palavras (as 2 páginas; alvo 900–1100 só avisa), bullet/`#list`, figura (`#figura-nebli`/`#figura-lateral`/`image(`), helper de E1 dentro da seção (a pré-aula não é mini-E1), qualquer interrogação (mesmo banimento de pergunta âncora/retórica da E1) ou menos de 15 termos plantados em negrito (avisa abaixo de 25). Escape documentado `--legado` **só** para regerar resumo histórico anterior à canonização — nunca em resumo novo. Canonizado em `CLAUDE.md` § "Antes da aula", `ROLES.md` § Redator diretriz 18, `.claude/commands/resumo.md` passo 6c e `ERROS.md` § tabela de checks (linha 24). Validado nos cinco caminhos de falha + no caminho feliz com o `pre-aula.typ` do `micro-01` (1009 palavras, 55 negritos, 0 interrogações).
...
- **Seção "Antes da aula" canonizada (pedido de Davi: "faltou o resumo pra ver antes da aula").** Todo resumo passa a abrir, **entre a capa e o sumário**, com 2 páginas de prosa contínua na voz NEBLI contando a história da aula e plantando os principais termos em negrito — **sem figura**, 900–1100 palavras, escrita DEPOIS da E1 (é destilada dela). Decisões de Davi nesta sessão: (a) vive **dentro do PDF do resumo**, não em arquivo separado; (b) **storytelling da aula com os principais termos**, não lista de objetivos nem vocabulário em bullets; (c) **nenhuma figura**. Wiring: helper `#pre-aula-page` no template (backup `backups/tecnicos/nebli_v2_apostila.typ.bak-2026-08-28-A1-pre-aula`), `gerar_main.render_pre_aula` (inclui só se `pre-aula.typ` existir → resumos antigos regeneram idênticos), `precompile-check.py` com `pre-aula.typ` em `EXPECTATIONS` + `pre-aula-page` em `TEMPLATE_HELPERS`, `CLAUDE.md` § "Antes da aula" + § Ordem do PDF renumerada, `ROLES.md` § Redator diretriz 18, `TEMPLATE_API.md`, `.claude/commands/resumo.md` passo 6c. Estreado em `correlacao-radio-pato-2` (41 páginas).
- **`ERROS.md` erro 21 registrado** — o contador de palavras da E1 do `auditar_pdf.py` delimita o miolo pela primeira ocorrência da palavra "Resumindo" e é enganado por `#mini-resumo` aberto com "Resumindo até aqui:". Warning falso-positivo em `correlacao-radio-pato-2` (acusou 726, o real era 7227). Correção definitiva (ancorar no banner) fica no backlog.
- **Resumo `correlacao-radio-pato-2` gerado** (Correlação radiopatológica do fígado, aula RD/PT de 2023, dial `profundidade: fundo`). 41 páginas (2 delas do "Antes da aula"), E1 com 19. Etapas 1-3 fechadas; deck-aula do passo 11 **não** rodou — sessão remota não alcança o AnkiConnect local.

### 2026-08-27
- **Calibração da UC03 pelas provas antigas — canonizada e escopada a UC03 (pedido de Davi: "estude as provas e organize pra os próximos resumos da UC03 saírem mais aprofundados do que cai na prova, sem misturar conteúdo de aulas diferentes"; e depois "organiza pra essa calibração ficar de forma inteligente do pipeline e só pra UC03").** Lidas as 30 provas do Drive de provas da UC03 (P1-P4 de 2015, 2016, 2017, 2018, 2019, 2023, 2024, 2025 + subs de 2015) e a planilha de cronograma do Davi. Entregues: (1) **`banco/aulas_uc03.yml`** — 76 aulas com slug, componente, código de área da prova (RD/PT/IM/MB/BM), nº de conteúdo oficial, data, prova e `chaves:` de recorte; P3/P4 `provisorio: true` (a tabela oficial da planilha só ia até o conteúdo 33). (2) **`referencias-externas/uc03/provas-questoes.json`** — 650 subquestões etiquetadas por área, com enunciado e gabarito. (3) **`referencias-externas/uc03/consultar.py`** — devolve, por slug, só as subquestões daquela aula (área etiquetada **E** palavra-chave da aula; marca `[eco]` quando a chave só aparece no gabarito). (4) **`referencias-externas/uc03/CALIBRACAO.md`** — DNA da prova (casos clínicos integrados, subquestão etiquetada por UMA área, tudo discursivo, gabarito premia mecanismo e traz bloco "curiosidade" que é exatamente o degrau a mais), régua "piso → teto da prova → degrau NEBLI", teto por componente e tabela por aula (já caiu · degrau na E1 · o que fica de fora). **Regra anti-mistura canonizada:** a prova integra o CASO, o resumo cobre a AULA; conteúdo de outra área entra no máximo como retomada de 1-2 frases, e a integração multiárea vive no motivador da E2/E3, nunca na E1. **Contrato de invisibilidade** igual ao `banco_slim`/`blueprint-step1`: consultado na redação, nada colado no PDF. Hooks: `CLAUDE.md` § References (1 bullet) + `.claude/commands/resumo.md` passo **2b condicional** ("se — e só se — a aula for de UC03") + menções nos passos 5 e 7. Nenhuma regra global mexida — outras UCs seguem idênticas.
