...
- **Resumo `bioq-26-acao-hormonal` — Ação hormonal (Bioquímica, slide "Controle Hormonal", Profa. Deborah/IQ-USP, 56 slides):** gerado em 2026-08-21 em sessão remota (cloud), etapas 1 a 3 + compilação. 38 páginas, 17 figuras do slide, 30 objetivas (20 MC + 10 C/E) + 5 discursivas. **Deck-aula NÃO montado** — AnkiConnect é localhost e a sessão remota não alcança; curadoria AnKing fica pendente para uma sessão local.
...
- **I9 — `auditar_pdf.py` delimita a E1 pelo primeiro "Resumindo" do texto extraído, e o helper `#mini-resumo` renderiza "Resumindo até aqui:".** Quando há mini-resumo cedo na PARTE I, o auditor corta o miolo ali e reporta contagem de palavras falsamente baixa (visto em `bioq-26-acao-hormonal`: acusou 714, real ≈7.600). Correção sugerida: delimitar pelo banner da página "Resumindo" (ou por `\nResumindo\n` isolado), não por substring solta. Só reporta — não bloqueia.
