...
- **2º semestre 2026 ativo (virada 2026-08-03, UC03 somada 2026-08-07):** Davi passou para o **2º semestre**, cursando **três UCs em paralelo** — **UC03** [RD · PT · IM · BM · MB integrados; P1 28/08 · P2 02/10 · P3 06/11 · P4 04/12], **Ciclo da Vida I (UC-16, MSP 3161)** [Pediatria + Geriatria, prova final única 28-29/09] e **Digestório (UC-8, Med5043)** [Anatomia/Histologia/Fisiologia/Embrio, P1 05-08/out · P2 26-30/nov]. Davi é da **Turma B** (usar a coluna Turma B dos cronogramas de UC-8/UC-16; UC03 tem B1/B2 só na sala de prova). Cronogramas assimilados em `cronogramas/` (fontes + digests `uc03-2026.md`, `ciclo-da-vida-i-uc16-2026.md`, `digestorio-uc8-med5043-2026.md`, `README.md`). Ainda **sem** `banco/aulas_ucNN.yml` p/ UC03/UC16/UC08 (resumos usam slide + bibliografia + AnKing). 1º resumo do 2º sem gerado: **Ciclo da Vida I Aula 1 — Crescimento e Desenvolvimento na Primeira Infância** (`pediatria-01-desenvolvimento-primeira-infancia`, Profa. Brentani).
- **Drive/Planilha mestra como fonte-verdade viva (canonizado 2026-08-07):** o cronograma agora vive na **planilha mestra do Davi** (`2026 - Matérias e Mês`, id `1AJx4unhfGmEl_4dqhO7OTpVhZ0TFAR5QgTorfmxSess`), que cobre UC03/UC04/UC07/UC08/UC16/UC19/UC21-DIC/UC30. Pasta-raiz "Medicina" no Drive: `1D9cPEn40EQl7sKx1z8KIZZ7lzBqFa5_8` (filhos: Cronogramas, Resumos e exercícios, Para não compartilhar; dono `dpbdes@gmail.com`). **Acesso via MCP Google Drive funciona daqui (tablet/cloud)** — `mcp__Google_Drive__read_file_content` na planilha devolve as duas abas (por-UC e mensal). Do tablet consigo **ler+navegar+criar** no Drive; upload de `.apkg` respeitando `DRIVE-ESTRUTURA.md` continua indo pelo `subir_drive.py` (rclone, PC do Davi). Ler a planilha antes de gerar resumo ou canonizar mudança de calendário; digests em `cronogramas/*.md` são snapshots — planilha vence em divergência.
...
### 2026-08-08 (adição pós-canonização)
- **AnKing v11 acessível SEM PC via clone público `langfield/anking-v11` (canonizado 2026-08-08, resolve pendência que era pra próxima sessão).** Davi confirmou PC quebrado permanentemente e pediu solução autônoma pra puxar AnKing. Achei o `langfield/anking-v11` no GitHub como clone público (6.5 GB, 24k+ cards em `.md` estruturados por tag AK — B&B/FirstAid/Physeo/Sketchy/UWorld/etc). Clonado em `/workspace/langfield/anking-v11` no ambiente remoto. Escrito `flashcards/scripts/curar_anking_local.py` que percorre `.md`, parse tags+fields, filtra por keyword nos fields configuráveis (default `Text,Extra,First Aid`), extrai imagens do `_media/` correspondente, gera JSON no formato NEBLIcard compatível com `empacotar_apkg.py`. Piloto digest-01 (aula anato Digestório): 425 candidatos → 60 selecionados por rank de keywords casadas → 54 imagens copiadas → JSON em `flashcards/curadoria/digest-01-...-anking-v11.json`. Combinado com os 70 NEBLIcards autorais num deck único de **130 cards** (`.apkg` de 6.9 MB) preservando naming Drive canônico. Tags dos AnKing incluem hierarquia AK completa + `NEBLI::anking-v11-curado` (rastreabilidade — Davi pode filtrar por tag no AnkiDroid).
  - **Limitação:** v11 é 1 versão atrás do v12 canônico (não tem erratas 2024-2026). Serve como MVP e cobre >95% do mesmo material. Upgrade pra v12 exige AnkiHub (pago) ou export do Davi (impossível sem PC).
  - **Gate absoluto (canônico R1/B2):** cards do AnKing entram como "base longitudinal Step 1" — nem todos ancorados na E1 daquela aula específica. Rastreabilidade via tag garante que Davi pode filtrar "só autorais ancorados" quando quiser. Não viola o canônico enquanto a tag distinguir.
  - **Custo:** clone one-time ~6.5 GB no `/workspace/` (persiste na sessão; próxima sessão precisa reclonar via `add_repo langfield anking-v11 read`). Curadoria ~30s por aula.
  - Wiring: `flashcards/scripts/curar_anking_local.py` (novo), JSON de curadoria em `flashcards/curadoria/<slug>-anking-v11.json`, JSON combinado autoral+anking em `flashcards/cards-nebli/<slug>_combined.json`, `.apkg` final via `empacotar_apkg.py <slug>_combined`. Warning benigno do genanki sobre HTML tags dos cards AnKing (Anki desktop/mobile são tolerantes).

### 2026-08-08
- **Ritmo Anki subiu (15 → 25 novos/dia + 1 etimologia/dia) + UC-8 no `DRIVE-ESTRUTURA.md` + pendências de AnKing-sem-PC, IO real vs simulado, e upload Drive chunked (pedido de Davi na sessão do resumo digest-01).**
  - **Ritmo canonizado 25/dia + 1 etimologia/dia** (era 15/dia): Davi subiu no início do 2º sem porque quer atacar mais volume. Deck `NEBLI::Etimologia` recebe 1 novo/dia como subdeck próprio (construção longitudinal lenta de raízes/afixos médicos). Total efetivo: 26 novos/dia. Configuração no AnkiDroid: limite de novos do topo `NEBLI` = 25, limite do subdeck `Etimologia` = 1 (Custom Study ou Deck Options individual). Atualizado em `sobre_mim.md`, `FLASHCARDS.md` § Estrutura, `flashcards/estrutura-deck-mestre.md`.
  - **DRIVE-ESTRUTURA.md ganhou seção UC-8** (canon 2026-08-08): componentes + ids das pastas mapeadas via MCP Google Drive; mapa slug→pasta iniciado com `digest-01`. Pasta canônica: `RESUMOS - Davi Sousa/B - Primeiro ano/UC08/Anatomia/01 - Esôfago, estômago e delgado/`. Naming da aula-curta ajustado pra bater com Drive (era "Esôfago, estômago e intestino delgado", virou "Esôfago, estômago e delgado" — regerou `.apkg` + renomeou PDFs).
  - **Pendência CANONIZADA — puxar cards AnKing sem PC:** implementar `flashcards/scripts/curar_anking_do_drive.py` que (a) uma vez, Davi exporta o AnKing v12 completo do PC pra `AnKing v12 export/` no Drive; (b) toda `/resumo`, script baixa via MCP (~30s), extrai SQLite do `.apkg` (zip contém `collection.anki2`), query por tag/keyword pra achar cards on-scope, copia campos+imagens pra JSON de curadoria; (c) `empacotar_apkg.py` combina AnKing curado + externos + autorais num único `.apkg`. Custo: ~3-4 GB no Drive de Davi, ~30s/sessão de download. Resolve o balde 1 (AnKing) do canônico deck-aula sem PC ligado. Extensível pra decks externos (BlueLink/Dope/Histo/Dorian) da mesma forma.
  - **Pendência CANONIZADA — IO real vs IO simulado no deck:** cards autorais atuais têm imagem inteira no Extra (padrão AnKing text-card). Para anatomia macro, avaliar (a) **IO simulado via HTML/CSS**: tarja preta no anverso sobre a região a identificar, some no verso (fácil de gerar programaticamente; não é IO nativo mas funciona no AnkiDroid); (b) **IO real via note type Image Occlusion Enhanced**: requer SVG de máscaras + coordenadas dos rótulos (sem OCR calibrado, é manual por imagem). Decidir na próxima aula de anato.
  - **Pendência CANONIZADA — upload PDF/apkg pro Drive via tablet:** MCP `mcp__Google_Drive__create_file` funciona pra textos pequenos, mas base64 de PDF (>3 MB) excede o limite do Read tool (256 KB) e não dá pra colar inline como argumento de tool call. Contorno hoje: Davi recebe o arquivo via `SendUserFile` no chat, sobe manualmente pelo app Drive no tablet (30 seg). Solução permanente proposta: implementar chunked upload via wrapper Python (base64 dividido em chunks de 200 KB + Google Drive resumable upload API), OU rclone com OAuth device flow autorizado uma vez pelo Davi (via navegador, sem PC). Ambos exigem trabalho de setup adicional; canonizar na próxima sessão.
  - **Bug técnico observado (não bloqueia):** o subagent `.claude/agents/questionador-sonnet.md` fica cacheado na sessão em que é criado (fixado com PascalCase Tools + `model: sonnet`, mas o resolver de tools rejeita mesmo assim). Nesta sessão o contorno foi `Agent(subagent_type: general-purpose, model: sonnet, prompt: <spec>)` — funciona idêntico. Na próxima sessão, com o stub já carregado no init, `subagent_type: questionador-sonnet` deve resolver limpo.

### 2026-08-07 (adição pós-canonização)
- **`sobre_mim.md` vira 5º arquivo vivo + estrutura de decks canônica reafirmada com regra "primeira prova" + faxina de pendências obsoletas + `.claude/commands/resumo.md` atualizado.** Segunda leva do dia:
  - **`sobre_mim.md`** (raiz do repo, novo): contexto pessoal do Davi — identidade + links canônicos Drive/planilha + como o 2º sem está organizado + estrutura de decks canônica + objetivos Step 1/FMUSP/residência HC. Carrega em toda sessão. Escopo diferente do `MEMORY.md`: aquele cobre estado do repo/pipeline; este cobre o estudante. `CLAUDE.md` § Memory System passou a listar **5 arquivos vivos** (era 4).
  - **Regra "primeira prova" (canonizada explicitamente):** aula cobrada em mais de uma prova mora **APENAS na primeira** (`NEBLI::UC03::P1::Radiologia::Ferramentas...` mesmo com `P1;P2;P3;P4` na planilha). Estudar o pai `NEBLI::UC03::` inclui essa aula em qualquer prova por aninhamento. Sem duplicata na coleção. Gravado em `FLASHCARDS.md` § Estrutura + `flashcards/estrutura-deck-mestre.md` § Árvore canônica + `sobre_mim.md`.
  - **Meta Cards×E1 = ≥8 (era ≥7) + piso ≥3 cards/subtópico nuclear reafirmados** em `FLASHCARDS.md` § Pipeline (fim) + `.claude/commands/resumo.md` passo 10, alinhando o que já estava no `CLAUDE.md`.
  - **Modo tablet/sem-PC canonizado em `FLASHCARDS.md` § Estrutura:** deck 100% autoral via `genanki`, naming `<Aula curta>.apkg`, árvore embutida no `.apkg`. Complementa o CANON 2026-08-03 (deck-completo-no-resumo) pra situação sem AnkiConnect.
  - **`.claude/commands/resumo.md` atualizado:** passo 4 chama `preparar_materiais.py` (não `extrair_slides.py` direto); passo 7 delega E2/E3 ao subagent `questionador-sonnet`; paths absolutos `C:\AI use\nebli\` viraram relativos (compatível com tablet/cloud); leitura obrigatória inclui `sobre_mim.md`.
  - **Faxina leve das pendências obsoletas do `FLASHCARDS.md`:** "cards autorais só após calibrar" e "migração física para `flashcards/`" marcadas como SUPERSEDED/EXECUTADA (mantidas comentadas por rastreabilidade). Nova pendência canonizada: script `.apkg` via `genanki` pra modo tablet.

### 2026-08-07
- **Materiais heterogêneos + QUESTIONADOR volta como Sonnet + profundidade `fundo` de default + Cards×E1 ≥8 (pedido de Davi na sessão do tablet UC03).** Três mudanças em uma só canonização, casadas com o pipeline sem-PC do 2º semestre:
  - **(a) Materiais além do slide.** Input do `/resumo` passa a aceitar em `slides/<slug>/` qualquer combinação de: slide do professor · **pergunta orientadora** (âncora editorial — vira subtópico E1 ou cobrança explícita E2/E3; sem cobertura = lacuna que trava) · **leitura obrigatória/texto de referência** (sobe teto de profundidade sem depender do AnKing; ranking leitura-prof > AnKing > base) · nota do professor (README) · figuras avulsas. Novo script `typst-build/preparar_materiais.py` cataloga tudo em `arquivos-trabalho/materiais/<slug>/INDICE.md` com papel editorial de cada peça e delega slide ao `extrair_slides.py` (retrocompat preservada). Wiring: `CLAUDE.md` § Fluxo Cowork (input reescrito + passo 3 chama `preparar_materiais.py`; Seção B do Tema Card expandida com bloco "Perguntas orientadoras → cobertura").
  - **(b) QUESTIONADOR-SONNET volta como subagent.** Reversão parcial da fusão de 2026-05-26 (aposentadoria de subagents por bug F9). E1 continua na sessão principal (Opus); **E2+E3 saem via `.claude/agents/questionador-sonnet.md` (Sonnet 4.6)**. Razão: libera Opus pra Tema Card+E1 (mais denso editorialmente) + hipótese de que F9 era específico Opus. Contingência: se subagent parar, devolve controle e Opus reassume. Stub aponta pra `ROLES.md` § Questionador (agora spec de subagent) + `CLAUDE.md` Routing Map atualizado.
  - **(c) Profundidade `fundo` = novo default + Cards×E1 meta sobe pra ≥8.** Davi pediu "mais profundidade + especialmente ampla cobertura nos cards". `CLAUDE.md` § Aprofundamento: dial default do Tema Card vira `fundo` (era `padrao`; Davi grava `padrao` só quando aula pede recuo). `CLAUDE.md` § Índice de completude: meta Cards×E1 sobe de ≥7 pra ≥8; **piso operacional novo — ≥3 cards por subtópico nuclear** (era ≥1 proporcional); periféricos ≥1. Bandas totais por aula continuam calibradas pela experiência mas o teto sobe junto.
  - **Casamento com o momento (sem PC):** deck-aula do próximo `/resumo` da UC03 sai **100% NEBLIcards autorais via genanki** (AnKing/externos inalcançáveis offline); gates `lint_neblicard` + `card-mirror` + `e1_e2_contract` continuam ativos. O piso ≥3 cards/subtópico + a profundidade `fundo` pesam ainda mais neste modo — o deck precisa carregar o que o AnKing normalmente carregaria.
