...
### Achados na corrida `biomol-04-integracao-metabolica-i` (2026-08-29)

- **`auditar_pdf_visual.py` bloqueia toda transição Resumindo → Etapa 2 por um lag de header que é do template, não do resumo.** O header de cada página é resolvido por `state("etapa").get()` no topo da página, antes do corpo; como `#etapa-header` e `#resumindo-page` chamam `set-etapa` **depois** do `pagebreak(weak: true)`, o cabeçalho de toda página de abertura de seção mostra o nome da seção **anterior**. Isso vale para as quatro transições (Sumário→E1, E1→Resumindo, Resumindo→E2, E2→E3), mas `TRANSICOES_TOLERADAS` só cobre duas delas — logo a transição Resumindo→E2 sai como ERROR que trava a movimentação do PDF. **Fix real (1 linha, exige aprovação do Davi por ser template):** em `etapa-header` e `resumindo-page`, chamar `set-etapa(...)` **antes** do `pagebreak(weak: true)`. **Fix paliativo:** acrescentar `("resumindo","etapa2")` e `("sumario","etapa1")` a `TRANSICOES_TOLERADAS`. Enquanto não decidido, a corrida verifica a página visualmente e move o PDF reportando o achado.
- **`validar_cores_e2_no_source` procura helpers da E4 aposentada.** Ele exige `#prova-consolidacao/integracao/aplicacao` em `etapa2.typ`, mas a E2 canônica usa `#questao-mc`/`#questao-ce` com `badge-consolidacao/integracao/aplicacao`. Resultado: warning falso em todo resumo novo. Trocar os três helpers procurados pelos badges.
- **Teto de 22 páginas da E1 × dial `profundidade: fundo`.** Este resumo fechou a E1 em 24 páginas (10.245 palavras) com a alavanca F4.1 já aplicada (figuras reduzidas para 50–66%). Como o canônico diz ao mesmo tempo "teto 22" e "completude > teto", vale o Davi decidir se o teto sobe quando o dial é `fundo` ou se o corte é obrigatório.
