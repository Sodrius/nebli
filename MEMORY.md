...
### 2026-08-27 · "Do zero" vira 2ª parte canônica do Resumindo
- **Pedido de Davi:** *"adiciona uma segunda parte do resumindo como canônica: vai ser um resumo 100% do zero para leigos do assunto, alguém que não leu a E1 e quer acompanhar melhor a aula. quero os principais termos dado de forma sucinta para eu ter um primeiro contato e acompanhar melhor as aulas."* O Resumindo passa a ter **duas partes com públicos opostos**: a Parte 1 (banner gold) continua sendo revisão para quem já leu a E1; a **Parte 2 "Do zero" (banner teal)** é primeiro contato, para ser lida *antes* da aula presencial por quem não sabe nada do tema.
- **Forma:** glossário de **15–30 termos**, 15–30 palavras cada, em linguagem do dia a dia, com `abertura` opcional de 3–5 linhas situando o leigo. **Ordem lógica de construção, nunca alfabética** — se o termo B depende do A, A vem antes, e a lista se lê de cima para baixo como escada. Sem sigla nova, sem `#termo-nota`, sem remissão a outro resumo. **É a única seção do NEBLI em que a definição precede o mecanismo**, porque a função é dar nome ao que o professor vai falar, não ensinar o mecanismo (isso é da E1). Heurística de cobertura: todo `#termo-nota` e toda `#sigla` da E1 merecem entrada, mais o que o professor usa em aula sem definir.
- **Implementação:** helper `#resumindo-zero-page(termos, abertura: none)` no template (backup `nebli_v2_apostila.typ.bak-2026-08-27-A1`), arquivo `typst-build/resumindo-zero.typ` entregue junto do `resumindo.typ`, e o `gerar_main.py` inclui a seção **sozinho quando o arquivo existe** — retrocompatível, resumo antigo sem o arquivo compila igual. Wiring canônico: `CLAUDE.md` § Resumindo (reescrita em 2 partes) + § Ordem do PDF item 4b, `ROLES.md` § Redator diretriz 5.bis, `typst-template/TEMPLATE_API.md`, `.claude/commands/resumo.md` passo 6.
- **Retrofit:** os dois resumos de UC3 já fechados nesta sessão ganharam a seção — `patologia-edema-congestao` (27 termos, 41 páginas) e `patologia-ambiental` (24 termos, 37 páginas).
- **Pendência do header-sync BAIXADA na mesma leva.** Como o `gerar_main.py` precisava ser alterado de qualquer forma para incluir a nova seção, o par `#pagebreak(weak: true)` + `#set-etapa(...)` entrou no `HEADER` de vez, antes de cada `#etapa-header`, do Resumindo, do Do zero e do gabarito. O header de página deixa de atrasar uma página em toda transição de seção, e o patch manual no `main.typ` não precisa mais ser reaplicado a cada corrida. Backup: `backups/tecnicos/gerar_main.py.bak-2026-08-27-dozero`.
