...
- **Imunologia — "Organização e funcionamento do sistema imune" (2026-08-21):** resumo gerado a partir do slide do Prof. José Alexandre M. Barbuto (26 slides, marcados "Unidade curricular-3"). Slug `imuno-01-organizacao-sistema-imune`; PDF `resumos-gerados/Organização do Sistema Imune - Etapas 1 a 3 - JAMB.pdf` (39 pp · E1 20 pp · 14 figuras · 30 objetivas · 5 discursivas). **Pendência de encaixe:** essa aula não está em `cronogramas/` (nem UC-16 nem UC-8 têm componente de Imunologia; a `imuno-01-disturbios-imunidade` de `banco/aulas_uc02.yml` é outra aula, de outro docente). Falta o Davi confirmar a UC/prova para (a) fechar a sigla do docente no nome do arquivo — usei JAMB por inferência — e (b) montar a árvore `NEBLI::<UC>::<Prova>::<Componente>::<Aula>` do deck.
