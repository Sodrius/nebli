...
- **Resumo `imuno-06-sistema-complemento` gerado** (Sistema complemento, videoaula de Imunologia da UC03, conteúdo 31, prova P2). Dial **`profundidade: fundo`** — Davi pediu "um nível um pouco acima do da prova". 46 páginas: capa · "Antes da aula" (2p) · sumário · E1 com 25 páginas, 3 PARTES e 13 subtópicos · Resumindo em 1 página · 30 objetivas (10 C/E, 3 "assinale a incorreta") · 5 discursivas · gabarito. 17 figuras, todas do slide. Passo 2b da calibração UC03 rodou (25 subquestões de IM no acervo 2015-2025; teto histórico = CH50/AH50 + C3/C4 localizando a via, e a E1 subiu o degrau no tioéster, na decisão próprio×estranho por fator H e nos quatro níveis de freio). **Quebra declarada:** E1 com 25 páginas contra o teto de 22 — mantida sob a cláusula "completude > teto" da diretriz 2 do Redator, por causa do dial `fundo`. **Deck-aula do passo 11 não rodou** — sessão remota não alcança o AnkiConnect local (mesmo bloqueio das corridas do fígado e do `micro-01`).
- **`arquivos-trabalho/rubrica-qualidade-didatica.md` estava desatualizada e vinha pontuando ao contrário.** Descoberto ao rodar a revisão Tier 2/Tier 3 do `imuno-06` à mão (subagentes indisponíveis na sessão remota). **T2.1** premiava "pergunta ingênua abrindo cada subtópico" — recurso **banido em 2026-05-29** —, então um resumo que segue o canônico atual tirava nota 1 num critério CRÍTICO. **T2.11** cobrava "figura ancorada na prosa", regra **removida em 2026-05-28**. Corrigidos: T2.1 reescrito para os 5 registros canônicos (com nota de inversão), T2.11 rebaixado a DESEJÁVEL e reorientado para julgar a legenda, T1.5 corrigido de 6–8 para 6–10 termo-notas, T1.7 marcado SUSPENSO (mindmap é no-op desde 2026-05-28). Lição de manutenção: a rubrica é prescritiva e não estava na lista de arquivos revisados quando uma regra editorial muda — vale incluí-la no passo 1 da § Atualização de regras canônicas.

- **Armadilha nova documentada — `ERROS.md` #25:** o header da página fica uma seção atrasado porque os helpers do template chamam `set-etapa` **depois** do `pagebreak`. Bloqueou o `auditar_pdf_visual` nesta corrida. Workaround por resumo (emitir `#set-etapa` antes de cada `#etapa-header`/`#include` no `main.typ`) aplicado e documentado; fix real no template fica pendente de aprovação de Davi.
