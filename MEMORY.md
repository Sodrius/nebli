...
### 2026-09-01
- **Resumo `biomol-05-operon-procariotos` gerado** (Regulação da expressão gênica em procariotos — o modelo do operon; Biologia Molecular da UC03, conteúdo 29, dial `profundidade: fundo` a pedido de Davi: "mais profundo que a prova, sem enrolar"). 43 páginas: capa · "Antes da aula" (2p, 1067 palavras, 42 negritos) · sumário · E1 com 21 páginas, 3 PARTES e 15 subtópicos · Resumindo em 1 página · 30 objetivas (20 MC + 10 C/E + 3 "assinale a incorreta") · 5 discursivas · gabarito. 22 figuras, todas do slide do professor. Paridade 20/20 PASS (média 1.04); C/E lidos item a item pós-redação conforme `ERROS.md` #23. Passo 2b da calibração UC03 rodou (7 subquestões de BM no acervo 2015-2025) e definiu o degrau: a prova cobra fenótipo, a E1 entrega o porquê — escape do promotor vs pouso da RNAP, problema do arranque resolvido pela expressão basal, alelos lacI^s e lacI^-d, eixo PTS → EIIA → adenilato-ciclase, exclusão do indutor, atenuação do trp e AraC como exceção. **Deck-aula do passo 11 não rodou** — sessão remota não alcança o AnkiConnect local (terceira corrida seguida com esse bloqueio: fígado, micro-01 e esta).
- **Corte de margem das lâminas extraídas (aplicado nesta corrida, candidato a virar etapa do `extrair_slides.py`).** Os PNGs do `extrair_slides.py` saem com as margens brancas do slide 16:9 e com o *placeholder* azul vazio que o template de PowerPoint do docente deixa no canto superior direito. Antes de redigir, as 42 lâminas foram limpas (região do placeholder pintada de branco + autocrop no bounding box de conteúdo, com 25 px de folga), com os originais preservados em `figuras/<slug>/_orig/`. Ganho real de área útil por figura — média de ~3509×2480 para ~2900×1950. **Pendência:** avaliar embutir esse passe no `extrair_slides.py` como flag (o placeholder é específico do template deste docente; o autocrop é genérico).
- **`ERROS.md` #22 confirmado e ampliado.** A sessão remota também não traz `poppler-utils` nem `PIL`/`numpy`. Receita usada: binário estático do typst + `pip install pymupdf pillow numpy` + os três shims (`pdftotext`, `pdfinfo`, `pdffonts`) sobre pymupdf. O shim de `pdftotext` precisa aceitar `-f`/`-l` e `-` como saída; o de `pdffonts`, imprimir as duas linhas de cabeçalho.
- **Bug pequeno do `verificar_gabarito_resumo.py` contornado (candidato a fix).** Vírgula final antes do fechamento de `#questao-mc` (`]),))`) faz o regex de `parse_etapa2` engolir a questão seguinte — nesta corrida, 3 vírgulas finais derrubaram a contagem de 30 para 27 e acusaram Q14/Q17/Q30 como ausentes. Removidas as vírgulas, passou. O fix real é tolerar `,` opcional antes de `))` no regex.
