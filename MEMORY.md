...
### 2026-08-24
- **Resumo `geriatria-envelhecimento-celular-tecidual` gerado (Etapas 1 a 3) — vídeo-aula de Patologia dentro de Ciclo da Vida I (UC-16), Prof. Luiz Fernando Ferraz da Silva.** 36 pp (E1 = pp. 4–20, dentro do teto 22), 12 figuras do slide (PARTE I 2 · PARTE II 4 · PARTE III 6), 9 termo-notas, 5 siglas. E2 com 20 MC + 10 C/E de 4 itens + 3 "assinale a incorreta"; **paridade 20/20 PASS** (banda 0.80–1.25, média 1.01, máx 1.16); gabarito sorteado A4·B4·C3·D4·E5 sem corrida ≥3, C/E 20C/20E sem CECE puro. Mapa de Confusões (48 entradas, 12 subtópicos) e mapa de Integração A+B gravados. Master 24,6 MB → leve 2,03 MB (12,1×). **Corrida em sessão remota (cloud):** sem Anki vivo, sem AnkiConnect e sem rclone — o **passo 11 (deck-aula) não rodou**; o passe de aprofundamento da E1 nasceu do slide + Robbins + bibliografia de Geriatria, não do AnKing. `banco/aulas_uc16.yml` continua inexistente e `blueprint-step1.md` não tem linha para o slug. Deck-aula fica pendente para uma sessão com Anki local.
- **Achado de pipeline (não corrigido, precisa de aval do Davi):** o header corrente do template tem **lag de uma página** em toda transição de seção — a página que traz o banner carrega o header da seção anterior (p3 banner "Etapa 1"/header SUMÁRIO; p21 banner "Etapa 2"/header RESUMINDO; p33 banner "Etapa 3"/header ETAPA 2). Causa: `set-etapa` roda no corpo da página e o `header:` do `pagina-padrao` resolve `state("etapa").get()` antes disso. É estrutural do `nebli_v2_apostila.typ` (vizinho da pendência já anotada nas linhas 986-990 do template) e afeta todo resumo. O `auditar_pdf_visual.py` o reporta como ERROR só no par resumindo→etapa2 (os outros pares escapam pela condição `h_atual == h_anterior`). Template **não** foi alterado — mudança visual exige amostra antes/depois aprovada por Davi.
- **Dois falsos-positivos de auditor confirmados nesta corrida:** (a) `auditar_pdf.py` conta "palavras da E1" delimitando pelo texto "Resumindo", e o prefixo do helper `#mini-resumo` ("Resumindo até aqui:") corta o miolo na primeira ocorrência — reportou 570 palavras onde o real é ~6,7 mil; (b) `auditar_pdf_visual.py` procura `#prova-consolidacao/-integracao/-aplicacao` em `etapa2.typ` para checar cor por categoria, mas esses helpers são da E4 (removida em 2026-05-22) — a cor hoje vem dos `badge-*`. Ambos merecem conserto na próxima sessão de manutenção dos auditores.
