...
- **2026-08-20 — 1º resumo de Imunologia (UC-3):** `imuno-17-reconhecimento-adaptativa` (Reconhecimento pelo Sistema Imune na Resposta Adaptativa, Prof. José Alexandre M. Barbuto), 41 páginas, E1 com 21 páginas e 13 figuras. Fonte foi a **transcrição da aula** (PDF com slide em página ímpar e fala do professor na par), não o slide solto — extração feita com script ad-hoc (imagem embutida por página ímpar + texto da par no `MAPA_CONTEUDO.txt`) e limpeza da miniatura de webcam e das barras pretas. Sem `banco/aulas_uc03.yml` e sem banco de questões para o slug: calibração veio do slide + bibliografia. Etapas 1 a 3 fechadas; deck-aula/AnKing NÃO rodou (sem AnkiConnect na sessão remota).
- **2026-08-20 — fix de header dessincronizado no `gerar_main.py`:** `#set-etapa(...)` passa a ser emitido ANTES de cada `#etapa-header(...)`, do `#include "resumindo.typ"` e do `#gabarito-page(...)`. Corrige o atraso de uma página no header de toda transição de seção (era ERROR bloqueante do `auditar_pdf_visual.py`). Template canônico intacto. Detalhe em `ERROS.md` erro 21; backup em `backups/tecnicos/gerar_main.py.bak-2026-08-20-header-sync`.
