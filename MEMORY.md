...
### 2026-08-09
- **Resumo + deck-aula `bioq-26-controle-hormonal` gerados do zero (Controle Hormonal, Bioquímica UC02, docente DS/Deborah Schechtman).** Aula nova, registrada em `banco/aulas_uc02.yml` (P4, ordem 31). PDF 41 pp (master 23,0 MB → leve 4,19 MB), 26 figuras do slide, 14 subtópicos em 3 PARTES + conclusão integradora. E2 com paridade **20/20 PASS** na banda dupla (Q13 e Q30 remediadas por enriquecimento da CORRETA, direção certa), gabarito A4·B4·C4·D4·E4, 10 C/E de 4 itens, 3 "assinale a incorreta". **Deck-aula: 88 NEBLIcards autorais** (`flashcards/decks-apkg/Controle Hormonal.apkg`).
- **Ambiente sem Anki e sem coleção AnKing** (container remoto): camadas 1 (AnKing curado) e 2 (decks externos) não puderam rodar; ordem de prioridade canônica ficou temporariamente invertida e está registrada como pendência em `flashcards/curadoria/bioq-26-controle-hormonal.md`. O apply via AnkiConnect foi substituído por **`.apkg` importável**, gerado por novo script `flashcards/scripts/montar_apkg_offline.py`.
- **Achado técnico reaproveitável:** o note type **AnKingOverhaul real** (model id `1659130414530`, com CSS e templates completos) foi extraído de `flashcards/decks-apkg/P3 - UC02 completo.apkg`, e agora os `.apkg` gerados offline importam direto no note type existente da coleção do Davi, sem duplicar modelo. Os mesmos `.apkg` são também a única fonte local de **cards AnKing reais** para lotes cegos do `card-mirror` quando o Anki está fora.
- **Bug de header do template corrigido no `main.typ` desta corrida (pendência para o `gerar_main.py`):** `etapa-header`/`resumindo-page`/`gabarito-page` fazem `pagebreak` e só então `set-etapa`, e o header da página lê o estado no início da página — resultado: header sempre atrasado uma seção (`auditar_pdf_visual` acusava ERROR "resumindo → etapa2"). Fix aplicado: emitir `#set-etapa(...)` **antes** de cada `#etapa-header(...)`/`#include resumindo` no `main.typ`. Convém migrar isso para o `gerar_main.py`.
