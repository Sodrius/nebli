...
5. **Gerar Tema Card** em `arquivos-trabalho/tema-card-<slug>.md` — Seções A (escopo), B (baseline aluno), C (profundidade + distribuição E2), D (Mapa de Confusões inicial), **Seção E (exemplares-âncora)**. **Canônico 2026-09-03:** a Seção A declara "piso 2, **teto 15** páginas" e, no lugar do aposentado `profundidade:`, duas linhas — *teto histórico* (o que a prova já cobrou) e *degrau* (onde a E1 vai além). A Seção B orça **8–10 subtópicos**, 450–550 palavras cada. Validar com Davi se houver ambiguidade (recorte slide × tema). **UC03:** a Seção A carrega também a linha de teto/degrau do passo 2b, e a Seção C declara a fronteira da aula (o que fica de fora por ser de outro slug).
...
7. **Redigir E2/E3** seguindo `ROLES.md` § Questionador. **Canônico 2026-09-03:** as questões de **Consolidação são decoreba** — recall direto de nome, número, unidade, sequência, classificação e qual-é-qual, com alternativas curtas de 10–20 palavras; o ângulo "Por que/Como" pertence à Integração. **UC03:** o acervo do passo 2b é fonte de distratora plausível-errada (o que o gabarito da FMUSP corrige como erro comum) e o verbo da E3 imita o da casa (*explique · descreva · justifique · qual o mecanismo mais provável*); o caso motivador pode ser multiárea como na prova — a E1 é que não pode. Para **Q01-Q30** reportar quadro completo de ratio de paridade (gate hard banda dupla 0.80-1.25 — canônico 2026-05-29).

8. **Compilar** via `python typst-build/gerar_main.py <slug>` → `python typst-build/precompile-check.py` (**bloqueia** fora de 8–10 subtópicos, acima de 6.000 palavras de miolo, ou com alternativas de Consolidação acima de 20 palavras em média — canônico 2026-09-03) → `cd typst-build && typst compile --root .. --font-path ../fonts main.typ <SLUG>.pdf` → `python typst-build/auditar_pdf.py <SLUG>.pdf`.

9. **Mover + compactar (CANÔNICO 2026-07-03, reforçado 2026-07-07 e 2026-09-03).** Rodar `python typst-build/pos_pipeline_check.py <pdf>` — ele **bloqueia a movimentação** se a E1 passar de 15 páginas; a exceção da § Missão exige `--quebra-declarada "motivo"`. Mover para `resumos-gerados/<SLUG>.pdf` APENAS se todos os auditores passarem. **Logo em seguida, SEMPRE gerar a cópia leve** (não é opcional): `cd typst-build && python comprimir_pdf.py "../resumos-gerados/<SLUG>.pdf" "../resumos-gerados/leves/<SLUG>.pdf"`. Política sidecar: o master fica full-res; a cópia leve (~2-3 MB) é a que se abre no celular / sobe pro Drive. Reportar os dois tamanhos no relatório final e, quando o Davi estiver via remote-control, enviar a cópia leve pelo chat (`SendUserFile`).
