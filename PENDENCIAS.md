...
## Consertos de pipeline achados em 2026-08-27 (resumo Edema e Congestão) — aguardando Davi

Três bugs sistêmicos apareceram na corrida de `patologia-edema-congestao`. Um foi corrigido na origem; os outros três esperam decisão porque tocam template/scripts canônicos.

- **[FEITO] Rótulos do gabarito consolidado eram fixos.** `gerar_main.py` → `render_gabarito` cravava `Q01–Q10 / Q11–Q25 / Q26–Q30`, o que só vale para a distribuição PADRÃO. Em PROFUNDO (8/17/5) e SUPERFICIAL (12-13/12-13/5) o PDF imprimia faixa errada em cima do próprio gabarito. Agora a faixa é derivada dos números presentes em cada bloco. Backup: `backups/tecnicos/gerar_main.py.bak-2026-08-27-edema`.

- **[FEITO 2026-08-27] Header de página atrasa uma página em toda transição de seção.** Resolvido no `gerar_main.py` junto da entrada do "Do zero" (o `HEADER` agora emite `#pagebreak(weak: true)` + `#set-etapa(...)` antes de cada seção). O template canônico não foi tocado nesse ponto. Registro histórico do diagnóstico abaixo.
  - Diagnóstico original: `#etapa-header` (em `nebli_v2_apostila.typ:117`) faz `pagebreak(weak: true)` e só depois `set-etapa(...)`; como o Typst resolve o header no início da página, a 1ª página de cada seção herda o nome da seção anterior. Vale para Sumário, Resumindo, E2, E3 e Gabarito — é o ERROR que `auditar_pdf_visual.py` reporta como "header dessincronizado", em todo resumo. Reprodução mínima sem conteúdo de resumo: `typst-build/_test_header.typ` (p2 mostra "ETAPA 1" com o banner "Etapa 2" no corpo). **Workaround aplicado só neste resumo,** direto no `main.typ`: `#pagebreak(weak: true)` + `#set-etapa("...")` antes de cada `#etapa-header`, antes do `#include "resumindo.typ"` e antes do `#gabarito-page`. Prova de que funciona: `typst-build/_test_header2.typ`. **Fix definitivo tem dois caminhos** — (a) emitir esse par no `HEADER` do `gerar_main.py`, que não toca o template canônico e conserta todo resumo novo; (b) mudar a ordem dentro do próprio `#etapa-header` no template, que também conserta os helpers `sumario`/`resumindo-page`/`gabarito-page` mas exige amostra antes/depois + backup, pela regra da pasta `typst-template/`. Recomendação: (a) agora, (b) quando houver janela para revisar o template.

- **[AGUARDA DAVI] `auditar_pdf.py` mede a E1 errado.** O check de "palavras da E1" delimita o miolo entre a 1ª ocorrência de "Etapa 1" e a 1ª de "Resumindo" — só que o helper `#mini-resumo` imprime "Resumindo até aqui:" no meio da PARTE I. Resultado: qualquer E1 que use `#mini-resumo` (ou seja, todas) é medida só até o primeiro mini-resumo e dispara warning de "miolo curto". Neste resumo acusou 1.005 palavras contra 7.220 reais. Fix sugerido: casar o **banner** da seção (texto em caixa-alta com tracking) em vez da string solta, ou delimitar por página a partir do `#resumindo-page`.

- **[AGUARDA DAVI] `auditar_pdf_visual.py` procura helpers da E4 removida.** O check de cores da E2 exige `#prova-consolidacao/integracao/aplicacao` em `etapa2.typ` — helpers da **Etapa 4, removida do pipeline em 2026-05-22**. A E2 usa `#badge-consolidacao/integracao/aplicacao`. O warning "helpers de cor faltando" dispara em 100% dos resumos desde então (conferido também no `etapa2.typ` de `radiologia-01-ferramentas-diagnostico`). Fix: trocar os alvos do check para os `badge-*`.
