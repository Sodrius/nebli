...
1. **Capa:** faixa navy com tema + subtítulo (vazio ou disciplina seca); bloco meta canônico (Disciplina · Onde estudar) — canônico 2026-05-26: "Nota de uso" removida do bloco meta a pedido de Davi; **"Onde aprofundar" REMOVIDO da capa (canônico 2026-07-07, a pedido de Davi 2× — vale para todo resumo gerado a partir de agora).** `gerar_main.render_capa` filtra qualquer linha meta cujo rótulo contenha "aprofundar"; o Orquestrador não emite mais essa linha. Rodapé identificador apenas na capa ("Davi Sousa — Turma 114" / "(61) 98264-7208 · dpbdes@gmail.com · PIX: dpbdes@gmail.com"). **Proibido no PDF qualquer página:** bloco/prova (P1/P2/P3), número/posição da aula, código de slug, turma, semestre, calendário.
   - **Bloco "Onde aprofundar" — SUSPENSO 2026-07-07 (não vai mais pra capa; parágrafo mantido só como histórico).** ~~(canônico 2026-06-30; ranking de completude 2026-07-01):~~ bloco da capa em **2 grupos** — (1) **Assistir/ler**: onde aprender o conteúdo (B&B/Bootcamp, NinjaNerd, Sketchy, UWorld, Pathoma); (2) **Dessuspender no AnKing v12**: 1 bullet por deck com **rótulo de completude** + **caminho de tag pra usar no Browse** + contagem de cards + 3-6 palavras do que cobre. **Os decks vêm ORDENADOS por quão completamente cobrem o tema (CANON 2026-07-01):** rótulos *Cobre melhor* (dessuspender primeiro) → *Complementa* → *Mais distante* (tangencial). Objetivo: guiar o Davi a ter, no deck dele, cards dessuspendidos que cubram o tema de cada aula — o mais completo primeiro, os distantes por último. Fonte real das tags via `buscar_tags_anking.py`. Fonte: `referencias-externas/onde-aprofundar.md` (preenchido pelo Orquestrador via `buscar_tags_anking.py`, que lê o export real `anking-v12-export.txt`). **Exceção escopada à regra de banimento de vocabulário Step 1:** os nomes dessas plataformas/decks são liberados **apenas neste bloco da capa**; em qualquer outra página (E1, Resumindo, E2/E3, gabarito) o vocabulário Step 1/USMLE/residência continua banido (ver `ROLES.md` Redator regra 10 + `precompile-check.py`). Recurso que não cobre o tema → "não cobre".
