...
6. `docs/canon/E1.md` — contrato didático, de completude e visual da E1;
...
- A E1 ensina a aula para compreensão inicial zero, sem perguntas retóricas e
  sem elos causais omitidos. `docs/canon/E1.md` é obrigatório e os exemplares
  UC02 calibram gesto pedagógico e voz.
...
2. E1 passa revisão de baseline zero, explicação passo a passo, fluidez,
   completude fonte→E1 e PDF visual renderizado;
3. todo card tem âncora E1;
4. **100% dos cards reais** passam no gate card a card;
5. `expected_card_count == validated_card_count == passed_card_count`;
6. manifesto `nebli-ankidroid-deck-v3` é gerado com nome canônico `NEBLI::<UC>::<Prova>::<Componente>::<Nome curto>`;
7. o Companion consegue instalar AnKing, decks externos, autorais, mídia nova e IO diretamente no AnkiDroid;
8. qualquer falha de runtime causa rollback das notas novas;
9. o recibo final confirma `installed_card_count == expected_card_count`.
