...
**5.bis. Pré-aula — 2ª parte do Resumindo (CANON 2026-08-27).** Junto do `resumindo.typ`, entregar também `pre-aula.typ`: **texto corrido de até 2 páginas** (~1.000–1.200 palavras) para quem **não leu a E1** e vai *assistir* a aula. Público oposto ao do Resumindo comum — aquele é revisão, esta é primeiro contato. **Prosa em voz NEBLI, não glossário nem lista:** os termos entram dentro da frase que os explica, na ordem em que o professor vai usá-los. Linguagem do dia a dia e zero pressuposto, mas com **aprofundamento correto** — simplificar não autoriza errar; se o mecanismo não cabe em linguagem simples, entrega o gesto certo e deixa o detalhe para a E1. **Negrito preto** no que o leitor leva embora (nome técnico na 1ª aparição, frase-chave de cada bloco). Sem sigla nova, sem `#termo-nota`, sem footnote, sem remissão a outro resumo. Heurística de cobertura: todo `#termo-nota` e toda `#sigla` da E1 aparecem na prosa, mais o que o professor usa em aula sem definir. Assinatura `#pre-aula-page[...]`; exemplar em `arquivos-trabalho/exemplares-pre-aula/`.
