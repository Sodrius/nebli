...
```
Seta o grupo de opções do deck **NEBLI** em **15 cards novos/dia** e teto alto de revisões (idempotente; `--dry-run` mostra antes/depois). Não importa quantos cards cada aula tenha — só 15 novos afloram por dia no topo `NEBLI`. Para cram de prova, usar preset separado no subdeck da prova (ex.: `NEBLI P3 hoje`), sem mexer no AnKing original.
...
## Reuso
`gerar_checklist.py` (passo 1 — semeia a checklist da E1: termo-notas + siglas por PARTE) · `buscar_tags_lote.py` (Camada 1 dirigida pela checklist — recall, puxa de todos os recursos) · `buscar_tags_anking.py` (Camada 1 por termo único, base reusada pelo lote) · `extrair_cards_anking.py` (Camada 2 — pool, com cache do dia) · `prefiltrar_pool.py` (Camada 2 — pool-slim + blocklist de patologia, menos tokens) · `verificar_cobertura_anking.py` (passo 5 — prova de cobertura X/Y + `cobertura-<slug>.json`) · `lint_neblicard.py` (gate anti-indução dos autorais) · `aplicar_curadoria_anking.py` (passo 6) · `configurar_ritmo_anking.py` (ritmo NEBLI: 15 novos/dia) · `liberar_p3_hoje.py` (cram temporário por prova) · `revisao_diaria_anking.py` (monitor) · `dessuspender_anking.py` (dessuspensão grossa por tag, legado do lote1).
