...
def main():
    ...
    with open(cur_path, encoding="utf-8") as f:
        spec = json.load(f)
    target = spec["target_deck"]
    ancora = spec.get("tag_ancora") or f"NEBLI::{args.slug}"
    kept = spec["kept"]

    pool_path = os.path.join(RAIZ, "arquivos-trabalho", f"anking-pool-{args.slug}.json")
    ...
    print(f"\nCriadas {criados} copias em '{target}' (tag {ancora}); {erros} erro(s).")
