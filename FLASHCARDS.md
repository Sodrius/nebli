...
**R13 — Gate pré-ship obrigatório (feedback 2026-07-15).** Antes de aplicar qualquer card autoral OU dessuspender qualquer card AnKing, rodar o **Checklist pré-ship** de `flashcards/ANTI-EXEMPLARES-CARDS.md`. Ele é o gate que impede a reincidência dos defeitos que o Davi já apontou — com destaque para os que mais se repetiram no lote de 2026-07-15: **imagem que não bate com o card** (A12 — o erro nº 1; imagem de tema não conta, na dúvida `img::substituir` em vez de shippar errada), **card sem Extra** (A13 — "todo card tem imagem + breve comentário"), **cloze sem dica de tipo** (A14 — `{{c1::x::process}}`), **excesso de tags** (A15) e **localização anatômica sem Image Occlusion** (A16). Padrões de forma já catalogados: A5–A11. Cada card reprovado no checklist não entra até ser consertado. Este gate é o "resolver definitivamente" pedido pelo Davi: os erros viram itens auditáveis, não repetição.

**R13 — Card autoral só entra se for AnKing-grade (canon 2026-07-14).** Feedback do Davi: os NEBLIcards prontos ainda parecem "cards criados pelo NEBLI", não AnKing. Correção: NEBLIcard autoral novo precisa seguir `flashcards/CARD-MIRROR-RUBRICA.md` e `flashcards/GUIA-IMAGENS-CARDS-NEBLI.md`: inglês por padrão, note type AnKingOverhaul, frente curta, cloze de 1–3 palavras no token de maior valor, mecanismo fora do stem, Extra curto em registro de nota de card, imagem quando visual, sem voz de apostila. Antes de aplicar: `lint_neblicard.py` + card-mirror quando o lote muda padrão. O aplicador preferido é `build_card.py`; `aplicar_neblicards.py` fica como compatibilidade legada sem mídia.
...
## Estrutura do deck de estudo + regra do AnKing intocado (canônico 2026-07-12, regra "primeira prova" explicitada 2026-08-07)

**Árvore única de estudo:** `NEBLI :: UC :: Prova :: Matéria :: Aula` (ex.: `NEBLI::UC02::P4::Bioquímica::Metabolismo dos aminoácidos`). "Matéria" e "componente" são sinônimos aqui — matéria é o rótulo do Davi, componente é o técnico da FMUSP. O topo `NEBLI` recebe outras UCs no futuro. Prova vem **antes** de matéria — então nunca existe um "Bioquímica P1–P4" agregado (o Davi não estudaria isso; cai em provas diferentes). Uso: estudar `NEBLI` inteiro = retenção longa (FSRS, **25 novos/dia + 1 novo/dia em `NEBLI::Etimologia`** — canon 2026-08-08, subiu de 15/dia); abrir `NEBLI::UC02::P4` = cravar antes daquela prova. Como é subdeck aninhado, **um card mora numa aula só** e estudar qualquer pai o inclui — o mesmo card serve retenção longa E cram de prova, com um agendamento FSRS único (Custom Study aumenta novos no deck-prova sem mexer no ritmo do geral).

**Regra da "primeira prova" (canonizada 2026-08-07):** quando o tema de uma aula cai em **mais de uma prova** (a planilha mestra usa notação `P1;P2;P3;P4`, ex.: "Ferramentas do diagnóstico radiológico" da UC03), o deck-aula mora **APENAS na primeira prova em que aparece** (`NEBLI::UC03::P1::Radiologia::Ferramentas do diagnóstico radiológico`, não em P2/P3/P4). Justificativa: (a) a aula é ensinada uma vez, então o Davi encontra o material dela na primeira prova; (b) para as provas seguintes, estudar `NEBLI::UC03::` (topo da UC) já inclui essa aula por aninhamento; (c) evita duplicata de card na coleção. Não há exceção — mesmo aula cobrada em 4 provas mora numa só.
...
**Sem Anki vivo (modo padrão quando PC do Davi está fora do ar):** os passos 2-4 rodam do export offline OU 100% autoral (quando o AnKing offline também não está disponível — caso do 2º sem 2026); montagem/apply (10-11) ficam pendentes até o Anki subir (a seleção + os autorais gerados ficam prontos em `flashcards/curadoria/<slug>.md` + `flashcards/cards-nebli/<slug>.json`). Com Anki vivo, o `/resumo` fecha o deck inteiro sozinho.

**Modo tablet / sem-PC + sem-AnkiConnect (canonizado 2026-08-07):** quando nem o Anki local nem o AnkiConnect estão alcançáveis (situação atual do 2º sem 2026 — Davi só tem tablet + AnkiDroid), o deck-aula sai **100% NEBLIcards autorais empacotados via `genanki`** num `.apkg` self-contained. Naming canônico do arquivo: `<Aula curta>.apkg` (mesmo nome curto do Drive/PDF). A árvore hierárquica `NEBLI::<UC>::P<N>::<Matéria>::<Aula>` já vai **embutida no `.apkg`** (via `Deck(deck_id, name="NEBLI::…")` do genanki) — quando Davi importa no AnkiDroid + sync AnkiWeb, o deck aparece diretamente no lugar certo. Gates `lint_neblicard` + `card-mirror` + `e1_e2_contract` continuam ativos como filtros de qualidade dos autorais. Prioridade AnKing→externos→autoral fica temporariamente comprometida (balde 1+2 vazios), mas o piso ≥3 cards/subtópico nuclear e o dial `fundo` da profundidade compensam — o deck autoral carrega o que o AnKing normalmente carregaria. Quando PC voltar, o deck existente pode ser enriquecido com AnKing curado por cima (adiciona, não substitui).

**Índice de completude Cards×E1 (0-10):** o X/Y COBERTOS da prova de cobertura vira uma das 3 notas do relatório de fechamento (ver `ROLES.md` § Índice de completude). **Meta ≥8 (canonizado 2026-08-07, subiu de ≥7); piso operacional ≥3 cards por subtópico nuclear, ≥1 por periférico.** Lacuna aponta fonte (AnKing → externo → autoral).
...
- **Rebuild NEBLIcards autorais existentes:** seguir `flashcards/CARD-MIRROR-RUBRICA.md` e o linter; prioridade: normalizar bioq-21/bioq-23, depois biomol-25, depois embrio.
...
- **Email de bandeira:** envio é opt-in e falha de LLM/SMTP preserva bandeiras. Ainda faltam smoke test SMTP/add-ons e validação da janela segura antes de produção. Estado operacional em `flashcards/README.md`.
...
- **Reviewer reescondendo card antes da nota — resolvido:** a causa observada em 2026-07-15 foi manutenção mutável via AnkiConnect durante a revisão, não FSRS. Scripts mutáveis devem abortar se `guiCurrentCard` retornar um card e aplicar mudanças em lote apenas fora do Reviewer.
...
- ~~**Cards NEBLI autorais:** só após calibrar; não gerar ainda~~ — **SUPERSEDED CANON 2026-08-03** (gerados no `/resumo`; card-mirror sempre por ora). Item resolvido; mantido comentado por rastreabilidade.
- **Script `.apkg` via genanki (modo tablet, sem AnkiConnect) — pendente para o próximo `/resumo`:** empacota os NEBLIcards autorais de `flashcards/cards-nebli/<slug>.json` num `.apkg` self-contained com árvore `NEBLI::<UC>::P<N>::<Matéria>::<Aula>` embutida via `genanki.Deck(deck_id, name="NEBLI::...")`. Naming `<Aula curta>.apkg`. Vai em `typst-build/empacotar_apkg.py` ou `flashcards/scripts/`. Escrever quando gerar o primeiro resumo do 2º sem em modo tablet.
...
- ~~**Migração física para `flashcards/`:** `git mv` de scripts + flashcards/curadoria + addon~~ — **EXECUTADA em 2026-07-10** (ver § Histórico). Item resolvido; mantido comentado por rastreabilidade.
